...
const VerificationOption: React.FC<VerificationOptionProps> = ({
  ...
  return (
    <button
      onClick={() => onSelect(id)}
      className={`flex overflow-hidden gap-3.5 self-stretch py-5 pr-9 pl-4 mt-4 rounded-lg ${
        isSelected
    ...
    >
      <img
        src={imageSrc}
        alt={`${title} icon`}
        className="object-contain shrink-0 aspect-square w-[62px]"
      />
      <div className="flex flex-col self-start items-start mt-1.5">
        <h3
          className={`self-start text-base font-bold leading-tight ${
            isSelected ? "text-green-800" : "text-zinc-700"
          }`}
        >
          {title}
        </h3>
    ...
    <div className="app_container">
      <section className="flex overflow-hidden flex-col justify-center items-center px-20 py-12 bg-white rounded-lg border-[0.5px] border-zinc-400 border-opacity-50 max-w-[624px] max-md:px-5 mx-auto my-auto">
        <div className="flex flex-col items-center max-w-full w-[365px]">
          ...
          <Button
            handleClick={() => router.push("/account-setup/profile/nimc")}
            className=" mt-8 !w-fit !py-3 !rounded-full !px-10"
          >
            Continue
          </Button>
        </div>
...
export default withAuth(VerificationProfile, {});
