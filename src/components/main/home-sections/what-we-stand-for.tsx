    ...
    <div className="flex flex-col w[33%] minh-[327px] max-md:ml-0 w-full z-10">
      <div className="flex overflow-hidden relative flex-col items-start px-8 pt-7 pb-20 w-full bg-white rounded-lg max-md:px-5 max-md:mt-8 lg:h-[440px]">
        <img
          src={iconSrc}
          alt=""
          className="object-contain rounded-md aspect-[1.11] w-[50px]"
        />
...
const WhatWeStandFor: React.FC<WhatWeStandForProps> = ({
  title,
  logoSrc,
  cards,
}) => {
      ...
      <div className="flex flex-col w-full max-md:max-w-full">
        <div className=" max-w-full text-3xl font-bold leading-none text-center text-white max-md:hidden">
          <img
            src={logoSrc}
            alt=""
            className="object-contain w-52 aspect-[2.5] absolute -left-5 "
          />
          <h1 className="self- text-center mt16 max-md:mt-10">{title}</h1>
        </div>
        <div className=" mt-20 mb-0 w-full max-lg:mt-0 max-lg:mb-2.5 max-lg:max-w-full">
          <div className="flex max-lg:gap-0 gap-5 max-lg:flex-col">
        ...
        <img
          src={"/images/what-we-stand-for-vector.svg"}
          alt=""
          className="object-contain self-end max-w-full aspect-[1.63] w-[130px] absolute bottom-[225px] max-md:hidden"
        />
      </div>
    </section>
  );
