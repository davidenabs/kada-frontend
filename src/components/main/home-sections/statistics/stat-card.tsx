    ...
    >
      <div className={`flex flex-col grow pb-9 w-full rounded-3xl ${bgColor} shadow-sm`}>
        ...
        <div className="flex flex-col items-start px-9 mt-8 max-md:px-5 ">
          ...
          {subtitle && (
            <p className={`mt-6 text-lg leading-6 ${accentColor || textColor}`}>
              {subtitle}
            </p>
          )}
          {subtitle2 && (
            <p className="text-[#7D7D7D] text-3xl mt-2">{subtitle2}</p>
          )}
          {source && (
            <p
              className={`absolute bottom-10 text-base leading-tight ${textColor} max-md:mt-10`}
            >
              {source}
            </p>
          )}
