...
export const useGetSubscriptionPlans = ({
  ...
  return useQuery<IResponse<IPaginatedResponse<ISubscription, "subscriptions">>, Error>({
...
export const useCreateSubscriptionPlanMutation = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [API_ENDPOINTS.GET_SUBSCRIPTION_PLANS] });
    },
  });
};
...
export const useUpdateSubscriptionPlanMutation = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ data, id }: { data: IUpdateSubscriptionPayload; id: string }) =>
