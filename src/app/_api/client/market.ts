...
import { ICreateMarketPayload, IUploadProductsFromSheetPayload } from "@/interface/market";
...
const marketClient = {
    ...
    createMarket: (data: ICreateMarketPayload): Promise<any> => ApiClient.post(
        API_ENDPOINTS.CREATE_MARKET,
        data
    ),
    updateMarket: (data: any, id: string): Promise<any> => ApiClient.put(
        API_ENDPOINTS.UPDATE_MARKET.replace(":id", id),
        data
    ),
    ...
    uploadProductsFromSheet: (data: IUploadProductsFromSheetPayload): Promise<any> => {
        const formData = objectToFormData(data);
        return ApiClient.post(
            API_ENDPOINTS.GET_MARKETS,
            formData,
